# DL_PY2F — Reference Manual (v1.3.0)
...
**Fortran-to-Python (beta, gfortran, Flang, and nvfortran).** Python loads an existing Fortran `.so` and parses `.mod` metadata to expose module variables, procedures, and derived types with dot syntax, without any changes to the Fortran source. The Intel `.mod` format is not supported yet.
...
The Fortran-to-Python path requires gfortran, Flang, or nvfortran because it parses their `.mod` file formats. The Python-to-Fortran path works with all listed compilers. See [section 3.2](#32-compatibility-and-limitations) for the full compiler compatibility matrix and minimum version requirements.
...
- **`dl.parseModule(modpath, padding=True, debug=False)`**
  Parses a single `.mod` file (gfortran gzip format, Flang text format, or nvfortran V35 ASCII format; auto‑detected). Returns a dictionary with per‑variable entries describing their type, shape, and layout. The `padding` argument controls whether struct‑alignment padding is applied; the default `True` is correct for most cases and should not normally be changed.
...
| Fortran-to-Python      | Yes      | No          | Yes    | Yes       |

The Fortran-to-Python path requires `.mod` files generated by gfortran, Flang, or nvfortran. The Intel `.mod` format is not supported.
...
- **Fortran-to-Python path supports gfortran, Flang, and nvfortran.** The Intel `.mod` format is not supported.
