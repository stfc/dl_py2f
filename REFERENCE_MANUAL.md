...
**Fortran-to-Python (beta, gfortran, ifx, Flang, and nvfortran).** Python loads an existing Fortran `.so` and parses `.mod` metadata to expose module variables, procedures, and derived types with dot syntax, without any changes to the Fortran source.
...
Both the Python-to-Fortran and Fortran-to-Python paths work with all listed compilers (gfortran, ifx, Flang, nvfortran). See [section 3.2](#32-compatibility-and-limitations) for the full compiler compatibility matrix and minimum version requirements.
...
| Fortran-to-Python      | Yes      | Yes         | Yes    | Yes       |

The Fortran-to-Python path supports `.mod` files generated by all four compiler families.
