...
There have been a few other tools for Python&ndash;Fortran coupling. A major category of them are interface generators, such as [`F2PY`](https://numpy.org/doc/stable/f2py/) [@peterson2009f2py] and its extensions, e.g., [`f90wrap`](https://github.com/jameskermode/f90wrap) [@Kermode2020-f90wrap], [`Scikit-build-core`](https://github.com/scikit-build/scikit-build-core), etc. These tools serve as builders which process Fortran source files and write out Python extension modules (via an intermediate C layer), and they put stress on calling specific Fortran functions/subroutines from Python. They often demand editing the original Fortran code, which, however, could be inconvenient or even unfeasible. In comparison, `DL_PY2F` does not require modifying the original Fortran source code and needs only a minimal interface that calls to the Fortran application's main routine invoking the whole library. Unlike in the `F2PY`-based tools, `DL_PY2F`'s support is not limited to the Fortran standards beyond 95 or Fortran derived types. [`gfort2py`](https://github.com/rjfarmer/gfort2py) is an ABI tool that works similarly as the Fortran-to-Python interoperability in `DL_PY2F`, also without modification to the source code while restricted to use `gfortran`. As opposed to `F2PY` and `gfort2py`, `DL_PY2F` does not provide compile tools which are only suitable for small pieces of Fortran code. Furthermore, `DL_PY2F` does not intervene in the Fortran applications' procedures, which normally involve a few parameters and are not compiled as exported symbols in shared objects. Instead, we focus on enabling modifications of application behaviour by manipulating computation data with inserted Python methods. To realise Python-to-Fortran data binding, one could also manually implement the mechanism based on the Python `ctypes` and `NumPy`'s `ctypeslib` modules. Such a challenging task might be indirectly assisted by tools such as [`CFFI`](https://github.com/cffi/cffi) which calls a Fortran-bound C code/library at the ABI/API level or [`SWIG+Fortran`](https://github.com/swig-fortran/swig) which generates Fortran 2003 wrappers to existing C/C++ libraries that are then used by Python. `DL_PY2F` has automated this mechanism by integrating an elaborate procedure which is portable with any modern Fortran compilers.
